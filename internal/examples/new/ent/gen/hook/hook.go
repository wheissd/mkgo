...
// The DefaultModelFunc type is an adapter to allow the use of ordinary
// function as DefaultModel mutator.
type DefaultModelFunc func(context.Context, *gen.DefaultModelMutation) (gen.Value, error)

// Mutate calls f(ctx, m).
func (f DefaultModelFunc) Mutate(ctx context.Context, m gen.Mutation) (gen.Value, error) {
	if mv, ok := m.(*gen.DefaultModelMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *gen.DefaultModelMutation", m)
}
