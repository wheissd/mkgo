...
type DefaultModelMutation struct {
	...
	oldValue      func(context.Context) (*DefaultModel, error)
	predicates    []predicate.DefaultModel
}

var _ ent.Mutation = (*DefaultModelMutation)(nil)

// defaultmodelOption allows management of the mutation configuration using functional options.
type defaultmodelOption func(*DefaultModelMutation)

// newDefaultModelMutation creates new mutation for the DefaultModel entity.
func newDefaultModelMutation(c config, op Op, opts ...defaultmodelOption) *DefaultModelMutation {
	m := &DefaultModelMutation{
		config:        c,
		op:            op,
		typ:           TypeDefaultModel,
...
// withDefaultModelID sets the ID field of the mutation.
func withDefaultModelID(id uuid.UUID) defaultmodelOption {
	return func(m *DefaultModelMutation) {
		var (
			err   error
			once  sync.Once
			value *DefaultModel
		)
		m.oldValue = func(ctx context.Context) (*DefaultModel, error) {
			once.Do(func() {
				if m.done {
					err = errors.New("querying old values post mutation is not allowed")
				} else {
					value, err = m.Client().DefaultModel.Get(ctx, id)
				}
			})
			return value, err
		}
...
// withDefaultModel sets the old DefaultModel of the mutation.
func withDefaultModel(node *DefaultModel) defaultmodelOption {
	return func(m *DefaultModelMutation) {
		m.oldValue = func(context.Context) (*DefaultModel, error) {
			return node, nil
		}
		m.id = &node.ID
	}
...
// operation is only accepted on creation of DefaultModel entities.
...
func (m *DefaultModelMutation) IDs(ctx context.Context) ([]uuid.UUID, error) {
	switch {
	...
	case m.op.Is(OpUpdate | OpDelete):
		return m.Client().DefaultModel.Query().Where(m.predicates...).IDs(ctx)
	default:
...
// OldName returns the old "name" field's value of the DefaultModel entity.
// If the DefaultModel object wasn't provided to the builder, the object is fetched from the database.
...
func (m *DefaultModelMutation) Where(ps ...predicate.DefaultModel) {
	m.predicates = append(m.predicates, ps...)
}

// WhereP appends storage-level predicates to the DefaultModelMutation builder. Using this method,
// users can use type-assertion to append predicates that do not depend on any generated package.
func (m *DefaultModelMutation) WhereP(ps ...func(*sql.Selector)) {
	p := make([]predicate.DefaultModel, len(ps))
	for i := range ps {
		p[i] = ps[i]
	}
	m.Where(p...)
...
func (m *DefaultModelMutation) Fields() []string {
	...
	if m.name != nil {
		fields = append(fields, defaultmodel.FieldName)
	}
	return fields
}
...
func (m *DefaultModelMutation) OldField(ctx context.Context, name string) (ent.Value, error) {
	...
	return nil, fmt.Errorf("unknown DefaultModel field %s", name)
}
...
func (m *DefaultModelMutation) SetField(name string, value ent.Value) error {
	...
	return fmt.Errorf("unknown DefaultModel field %s", name)
}
...
func (m *DefaultModelMutation) AddField(name string, value ent.Value) error {
	...
	return fmt.Errorf("unknown DefaultModel numeric field %s", name)
}
...
func (m *DefaultModelMutation) ClearField(name string) error {
	return fmt.Errorf("unknown DefaultModel nullable field %s", name)
}
...
func (m *DefaultModelMutation) ClearEdge(name string) error {
	return fmt.Errorf("unknown DefaultModel unique edge %s", name)
}

// ResetEdge resets all changes to the edge with the given name in this mutation.
// It returns an error if the edge is not defined in the schema.
func (m *DefaultModelMutation) ResetEdge(name string) error {
	return fmt.Errorf("unknown DefaultModel edge %s", name)
}
